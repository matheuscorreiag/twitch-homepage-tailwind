...
const ChannelCards = ({ imageUrl, views, channelName, game, title }: ChannelCardsProps) => {
  ...
  // const minimizedTitle = title.length > 35 ? `${title.substring(0, 35)}...` : title;
  return (

    <div className='w-[300px] h-40 p-2 image' >
      ...
      <div className='grid grid-flow-col grid-cols-[35px] mt-2 '>
        ...
        <div className='flex-col ml-4 text-white  w-[230px] flex '>
          ...
          <span className='font-thin text-zinc-400 text-xs cursor-pointer'>{channelName}</span>
          <span className='font-thin text-zinc-400 text-xs mt-1 cursor-pointer'>{game}</span>
